  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-[#EEF1F6]">
      <div className="max-w-7xl mx-auto flex items-center gap-2.5 px-4 sm:px-6 lg:px-8 py-3">
        <Link to="/" className="flex items-center gap-2 md:flex-none flex-1" onClick={() => setOpen(false)}>
          <img src={logo} alt="VL" className="w-[34px] h-[34px] object-contain" />
        ...
        <button
          ...
          aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
          className="md:hidden w-10 h-10 border border-[#E4E9F1] rounded-[10px] bg-white flex flex-col gap-1 items-center justify-center cursor-pointer"
        >
      ...
      {open && (
        <nav className="md:hidden bg-white border-t border-[#EEF1F6] px-4 pt-2 pb-4 flex flex-col gap-0.5 animate-fadeup">
          {NAV_ITEMS.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
