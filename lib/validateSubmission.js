...
const ALLOWED_FIELDS = {
  name: 120,
  email: 200,
  // Room for a number still carrying +92, spaces or brackets. It is reduced to eleven digits
  // by normalisePhone() below before it is checked.
  phone: 24,
  nation: 80,
  qual: 60,
  grade: 40,
...
const EMAIL = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/
// Eleven digits starting with a zero, which is what a Pakistani number is. The browser strips
// everything else as the student types, but the endpoint is public, so the same rule has to be
// enforced here too or a bot can post whatever it likes straight into your inbox.
const PHONE = /^0\d{10}$/

// Accepts a number that arrived in another shape (+92, 0092, spaces, dashes) and reduces it to
// the local eleven digit form before it is checked, so a genuine submission from an old cached
// copy of the page is not thrown away.
...
export function validateSubmission(raw) {
  ...
  if (fields.phone) {
    ...
    if (!PHONE.test(fields.phone)) {
      return { ok: false, error: 'Please enter your full 11 digit number starting with 0, for example 03215208625.' }
    }
  }
